...
int main()
{
	...
	bool sharkIsReady = false;
	int sharkTime = 3; //! Number of iterations before shark can breed //! 
	int fishTime = 2; //! Number of iterations before fish can breed //!
	bool fishIsReady = false;
	
	int SharkBreed = 0;//!breeding timer to shark.h //!
	int FishBreed = 0;//!breeding timer for fish to Fish.h //!
	
	int babyFishes = 0; //!Current number of fishes divided by 2
	int babySharks = 0;
	int starve = 7; //! Number of iterations before shark starves and is deleted from vector //! 
	...
	//! Life Cycle //!
	while (counter <= Timer && NumFish < 100)//! changed to j as i was being used elsewhere //!
	    {
	        ...
	        while (!sharkBesideFishResult.empty())//! while theres still a value in the vector //!
		{
	      ...
	      for (i = 0; i < sharkTracker.size(); i++)
		{
		  int shB = sharkTracker[i].getBreedTimer();//!just saves the breed timer from the object into sharkIsReady
		 
		  if(shB != 0 && shB % sharkTime == 0)//!if the shark breed object variable is a multiple of 5 and not a value of 0, enter
		    {
		      sharkIsReady = true;
		    }
		  
		  if(sharkIsReady == true )
		    {
		       while (slot != 'E')//! find position in grid for new shark object //!
			{
			  row = rand() % 19;
			  ...
			  if (space = '_')
		        ...
		        slot = ' ';
			sharkIsReady = false;
			sharkTracker[i].setBreedTimer(SharkBreed);
	      ...
	      for (i = 0; i < fishTracker.size(); i++)
		{
		  int fB = fishTracker[i].getBreedTimer();//!just saves the breed timer from the object into sharkIsReady
		  if(fB != 0 && fB % fishTime == 0)//!if the shark breed object variable is a multiple of 5 and not a value of 0, enter
		    {
		      fishIsReady = true;
		    }
		  ...
		  if(fishIsReady == true )
		    {
		      while (slot != 'E')//! find position in grid for new fish object //!
			 {
			   ...
			   if (space = '_')
			     {
			       grid[row][col] = 'F';
			       Fish f(FishBreed, row, col); //! Create fish object //!
			       f.setBreedTimer(0);
			       fishTracker.push_back(f); //! add to tracker //!
			       NumFish++;
			       slot = 'E';
		        ...
		        slot = ' ';
			fishIsReady = false;
			fishTracker[i].setBreedTimer(FishBreed);			
		    }
		   else
		    {
		      fishTracker[i].setBreedTimer(FishBreed); 
